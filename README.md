...
- 运行时**零第三方依赖**：GUI 用 `tkinter`，逻辑用标准库；仅"删除进回收站"使用 `send2trash`（打包时自动包含）。
- **删除安全设计**：所有删除操作优先把文件移到**回收站**（`send2trash`），误删还能找回；回收站不可用的环境才回退到直接删除。回收站操作失败（如文件被占用）会统计为失败，**绝不降级成永久删除**。
- 回收站（回收站）本身不直接删文件，而是通过 `ctypes` 调用 `shell32.dll` 的
  `SHQueryRecycleBinW`（查大小）和 `SHEmptyRecycleBinW`（清空），更规范安全。
- 扫描时被占用/无权限的文件会跳过统计，显示的数字可能比实际略小（所有清理工具的共性）。
